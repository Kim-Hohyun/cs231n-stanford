...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, dim in enumerate(hidden_dims):
            self.params['W'+str(i+1)] = np.random.randn(ex_dim, dim) * weight_scale
        ...
        self.params['W'+str(self.num_layers)] = np.random.randn(ex_dim, num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        if self.normalization=='batchnorm':
            for i in range(1, self.num_layers):
                out, cache[i] = affine_bn_relu_forward(out, self.params['W'+str(i)], self.params['b'+str(i)],
                                                       self.params['gamma'+str(i)], self.params['beta'+str(i)],
                                                       self.bn_params[i-1])
                
        else:
            for i in range(1, self.num_layers):
                out, cache[i] = affine_relu_forward(out, self.params['W'+str(i)], self.params['b'+str(i)])
                if self.use_dropout:
        ...
        for i in range(1, self.num_layers+1):
            L2reg += np.sum(self.params['W'+str(i)] * self.params['W'+str(i)])
        ...
        if self.normalization=='batchnorm':
            for i in range(1, self.num_layers)[::-1]:
                dA, grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = affine_bn_relu_backward(dA, cache[i])
        else:
            for i in range(1, self.num_layers)[::-1]:
                if self.use_dropout:
                    dA = dropout_backward(dA, dropout_cache[i])
                dA, grads['W'+str(i)], grads['b'+str(i)] = affine_relu_backward(dA, cache[i])
